...
interface AttestationTemplateArProps {
  attestation: any
  showGrade: boolean
  showFonction: boolean
}

export default function AttestationTemplateAr({ attestation, showGrade, showFonction }: AttestationTemplateArProps) {
  const iframeRef = useRef<HTMLIFrameElement>(null)

  useEffect(() => {
    const loadTemplate = async () => {
      try {
        let templatePath = "/templates/attestation-simple-ar.html"

        if (showGrade && showFonction) {
          templatePath = "/templates/attestation-grade-fonction-ar.html"
        } else if (showGrade) {
          templatePath = "/templates/attestation-grade-ar.html"
        }
            ...
            setTimeout(() => {
              if (iframeDoc.body) {
                ...
                if (!showGrade) {
                  ...
                  if (birthPlaceElement) birthPlaceElement.textContent = attestation.lieuNaissanceAr || ""
                }
                ...
                if (showGrade) {
                  ...
                  if (gradeElement) gradeElement.textContent = attestation.gradeAr || ""
                }
                ...
                if (showGrade && showFonction) {
                  ...
                  if (fonctionElement) fonctionElement.textContent = attestation.fonctionAr || ""
                }
                ...
                if (referenceElement) {
                  referenceElement.textContent = `${attestation.id.split("-")[1]} / م م ن / م و ع إ آ / ${new Date().getFullYear()}`
                }
              }
            }, 100)
          }
  ...
  return (
    <div className="relative w-full h-[842px] border rounded-md overflow-hidden">
      <iframe ref={iframeRef} className="w-full h-full" title="شهادة عمل" />
      <Button
        onClick={handlePrint}
        className="absolute top-4 left-4 bg-primary text-white px-4 py-2 rounded-md hover:bg-primary/90 flex items-center gap-2"
      >
